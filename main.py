...
# Pre-calculate sin and cos values for all rays for better performance
RAY_COS_LOOKUP = []
RAY_SIN_LOOKUP = []
for ray in range(NUM_RAYS):
    ray_angle = -FOV / 2 + ray * DELTA_ANGLE
    RAY_COS_LOOKUP.append(math.cos(ray_angle))
    RAY_SIN_LOOKUP.append(math.sin(ray_angle))
...
# Create static sky and floor surfaces for better performance
horizon_y = WIN_HEIGHT // 2

# Create sky gradient surface
sky_surface = pygame.Surface((WIN_WIDTH, horizon_y))
sky_color_top = pygame.Color(SKY_COLOR_TOP)
sky_color_bottom = pygame.Color(SKY_COLOR_BOTTOM)
for y in range(horizon_y):
    t = y / horizon_y  # 0 at top, 1 at horizon
    color = sky_color_top.lerp(sky_color_bottom, t)
    pygame.draw.line(sky_surface, color, (0, y), (WIN_WIDTH, y))

# Create floor gradient surface
floor_surface = pygame.Surface((WIN_WIDTH, WIN_HEIGHT - horizon_y))
floor_color_top = pygame.Color(FLOOR_COLOR_TOP)
floor_color_bottom = pygame.Color(FLOOR_COLOR_BOTTOM)
for y in range(WIN_HEIGHT - horizon_y):
    t = y / (WIN_HEIGHT - horizon_y)  # 0 at horizon, 1 at bottom
    color = floor_color_top.lerp(floor_color_bottom, t)
    pygame.draw.line(floor_surface, color, (0, y), (WIN_WIDTH, y))
...
def draw_background(screen):
    """Draw sky and floor with gradient effects using pre-rendered surfaces."""
    horizon_y = WIN_HEIGHT // 2
    
    # Blit pre-rendered sky and floor surfaces
    screen.blit(sky_surface, (0, 0))
    screen.blit(floor_surface, (0, horizon_y))

def raycasting(screen):
    # Define step size for higher precision ray casting
    STEP_SIZE = 3
    
    # Pre-calculate player angle cos and sin
    player_cos = math.cos(player_angle)
    player_sin = math.sin(player_angle)
    
    # a loop for each ray
    for ray in range(NUM_RAYS):
        # Get pre-calculated ray direction relative to player angle
        ray_cos = player_cos * RAY_COS_LOOKUP[ray] - player_sin * RAY_SIN_LOOKUP[ray]
        ray_sin = player_sin * RAY_COS_LOOKUP[ray] + player_cos * RAY_SIN_LOOKUP[ray]
        ...
        for depth in range(int(MAX_DEPTH / STEP_SIZE)):
            ...
            if is_wall(target_x, target_y):
                ...
                # Determine wall face (vertical or horizontal) based on proximity to grid lines
                offset_x = surface_x % TILE_SIZE
                offset_y = surface_y % TILE_SIZE
                
                # Distance to nearest vertical grid line
                dist_vertical = min(offset_x, TILE_SIZE - offset_x)
                # Distance to nearest horizontal grid line
                dist_horizontal = min(offset_y, TILE_SIZE - offset_y)
                ...
                # Correct distance for fish-eye effect using dot product
                corrected_depth = actual_depth * (player_cos * ray_cos + player_sin * ray_sin)
